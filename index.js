...
const enrollments = JSON.parse(
  fs.readFileSync(path.join(__dirname, "data/enrollments.json"))
);
...
function createRoutes(basePath, data) {
  app.get(
    `${basePath}`,
    withDelay(1, (req, res) => res.json(data))
  );
  app.get(
    `${basePath}/500`,
    withDelay(500, (req, res) => res.json(data))
  );
  app.get(
    `${basePath}/1000`,
    withDelay(1000, (req, res) => res.json(data))
  );
  app.get(
    `${basePath}/1500`,
    withDelay(1500, (req, res) => res.json(data))
  );
  app.get(
    `${basePath}/2000`,
    withDelay(2000, (req, res) => res.json(data))
  );
  app.get(
    `${basePath}/2500`,
    withDelay(2500, (req, res) => res.json(data))
  );
  app.get(
    `${basePath}/3000`,
    withDelay(3000, (req, res) => res.json(data))
  );
}
